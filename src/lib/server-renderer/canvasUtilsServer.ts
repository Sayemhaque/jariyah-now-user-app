...
export function drawOverlayLayer(
  ...
  switch (style) {
    ...
    case 'bottom-gradient':
      ...
      bg.addColorStop(0.55, `rgba(${rgb},${alpha * 0.5})`)
      bg.addColorStop(1, `rgba(${rgb},${alpha})`)
      ctx.fillStyle = bg
      ctx.fillRect(0, 0, w, h)
      break
    case 'top-gradient':
      const tg = ctx.createLinearGradient(0, 0, 0, h)
      tg.addColorStop(0, `rgba(${rgb},${alpha})`)
      tg.addColorStop(0.55, `rgba(${rgb},${alpha * 0.5})`)
    ...
    case 'vignette':
      ...
      vg.addColorStop(0.35, `rgba(${rgb},0)`)
      vg.addColorStop(0.75, `rgba(${rgb},${alpha * 0.55})`)
      vg.addColorStop(1, `rgba(${rgb},${alpha})`)
      ctx.fillStyle = vg
      ctx.fillRect(0, 0, w, h)
      break
    case 'center-focus':
      const cg = ctx.createRadialGradient(w / 2, h / 2, 0, w / 2, h / 2, Math.max(w, h) * 0.6)
      cg.addColorStop(0, `rgba(${rgb},${alpha})`)
      cg.addColorStop(0.45, `rgba(${rgb},${alpha * 0.5})`)
      cg.addColorStop(0.8, `rgba(${rgb},0)`)
      ctx.fillStyle = cg
      ctx.fillRect(0, 0, w, h)
      break
  }
...
export function loadImage(url: string): Promise<any> {
  if (url.startsWith('/') && !url.startsWith('http://') && !url.startsWith('https://')) {
    ...
    const fsPath = candidates.find((c) => existsSync(c))
    if (fsPath) return canvasLoadImage(fsPath)
  }
...
export function setupCanvas(
  width: number,
  height: number,
): { canvas: any; ctx: CanvasRenderingContext2D } {
